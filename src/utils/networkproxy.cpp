...
NetworkProxy NetworkProxyHelper::getSystemProxyWindows()
{
    ...
    if (internetSettings.contains("AutoConfigURL")) {
        ...
        proxy.setPort(0);
    } else if (internetSettings.value("ProxyEnable").toInt() == 1 && internetSettings.contains("ProxyServer")) {
...
void NetworkProxyHelper::resetSystemProxyLinuxGnome()
{
    ...
    p.waitForFinished();
    p.start("gsettings", QStringList { "set", "org.gnome.system.proxy.http", "enabled", "false" });
    p.waitForFinished();
    p.start("gsettings",
...
NetworkProxy NetworkProxyHelper::getSystemProxyLinuxKde()
{
    QProcess p;
    NetworkProxy proxy;
    p.start("kreadconfig5", QStringList { "--file", "kioslaverc", "--group", "Proxy Settings", "--key", "ProxyType" });
    p.waitForFinished();
